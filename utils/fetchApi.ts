...
export async function fetchApes(
  ...
  try {
    let query = supabase
      ...
      .select(
        "*, hrm_users:hrm_user_id(id,firstname,middlename,lastname), hrm_ape_diagnoses(id,ape_id,diagnosis,diagnosis_date,diagnosed_by,diagnosed_by_user:diagnosed_by(id,firstname,middlename,lastname),hrm_ape_diagnosis_files(id,diagnosis_id,file_name,file_path))",
        { count: "exact" },
      )
      .eq("org_id", process.env.NEXT_PUBLIC_ORG_ID);
