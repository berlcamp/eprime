...
// Types
import type { NosaTypes, SignatoriesTypes } from '@/types'
...
const Page: React.FC = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<NosaTypes | null>(null)

  const [list, setList] = useState<NosaTypes[]>([])
  const [filterUser, setFilterUser] = useState<string>('')

  const [perPageCount, setPerPageCount] = useState<number>(10)
  const [editData, setEditData] = useState<NosaTypes | null>(null)
  ...
  const fetchData = async () => {
    ...
    try {
      const result = await fetchNosa({ filterUser }, perPageCount, 0)
  ...
  const handleShowMore = async () => {
    ...
    try {
      const result = await fetchNosa({ filterUser }, perPageCount, list.length)
  ...
  const handleEdit = (item: NosaTypes) => {
    setShowAddModal(true)
    setEditData(item)
  }
  ...
  const handleInitiatePrint = (item: NosaTypes) => {
    setSelectedItem(item)
    setShowSignatoriesModal(true)
  }

  const handlePrint = (item: NosaTypes, signatories: SignatoriesTypes) => {
