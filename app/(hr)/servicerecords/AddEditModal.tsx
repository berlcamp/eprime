...
const AddEditModal = ({ hideModal, editData, userId }: ModalProps) => {
  ...
  const { register, formState: { errors }, reset, handleSubmit } = useForm<ServiceRecordTypes>({
    mode: 'onSubmit'
  })
  ...
  const handleCreate = async (formdata: ServiceRecordTypes) => {
    ...
    try {
      ...
      if (error) {
        void logError('Create service record', 'hrm_service_records', JSON.stringify(newData), error.message)
  ...
  const handleUpdate = async (formdata: ServiceRecordTypes) => {
    ...
    try {
      ...
      if (error) {
        void logError('Update service record', 'hrm_service_records', JSON.stringify(newData), error.message)
        setToast('error', 'Saving failed, please reload the page and try again.')
        throw new Error(error.message)
      }
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="app__modal_body">
            ...
            <div className='app__form_field_container'>
              <div className='app__label_standard'>
                <span>Employee: </span>
                <span className='font-bold'>{user.firstname} {user.middlename} {user.lastname}</span>
              </div>
            </div>
            <div className='flex space-x-4 w-full'>
                  ...
                  <div className='w-full'>
                    ...
                    <div>
                      <input
                        ...
                        placeholder='Status'
                        className='app__select_standard'/>
                      {errors.status && <div className='app__error_message'>Status is required</div>}
                    </div>
                  </div>
                </div>
                <div className='app__form_field_container'>
                  <div className='w-full'>
                    ...
                    <div>
                      <input
                        ...
                        placeholder='Salary'
                        className='app__select_standard'/>
                      {errors.salary && <div className='app__error_message'>Salary is required</div>}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className='w-1/2'>
                  ...
                  <div className='w-full'>
                    ...
                    <div>
                      <input
                        ...
                        placeholder='Station'
                        className='app__select_standard'/>
                      {errors.station && <div className='app__error_message'>Station is required</div>}
                    </div>
                  </div>
                </div>
                <div className='app__form_field_container'>
                  <div className='w-full'>
                    ...
                    <div>
                      <input
                        ...
                        placeholder='Branch'
                        className='app__select_standard'/>
                      {errors.branch && <div className='app__error_message'>Branch is required</div>}
                    </div>
                  </div>
                </div>
                <div className='app__form_field_container'>
                ...
                <div className='app__form_field_container'>
                  <div className='w-full'>
                    <div className='app__label_standard'>Separation Cause</div>
                    <div>
                      <input
