...
const AddEditModal = ({ hideModal, editData }: ModalProps) => {
  ...
  const handleCreate = async (formdata: ItemTypes) => {
    ...
    try {
      ...
      if (error) {
        void logError('Create New Item', 'hrm_items', JSON.stringify(newData), error.message)
      ...
      const hrmSchool = formdata.school_id !== '' ? schools.find(s => s.id.toString() === formdata.school_id) : null
      const updatedData = { ...newData, hrm_school: hrmSchool, hrm_position: hrmPosition, hrm_user: user ?? null, id: data[0].id }
  ...
  const handleUpdate = async (formdata: ItemTypes) => {
    ...
    try {
      ...
      if (error) {
        void logError('Update item', 'hrm_items', JSON.stringify(newData), error.message)
        setToast('error', 'Saving failed, please reload the page and try again.')
        throw new Error(error.message)
      }
      ...
      const updatedData = { ...newData, hrm_school: hrmSchool, hrm_position: hrmPosition, hrm_user: user ?? null, id: editData.id }
      ...
      <div className="app__modal_wrapper2">
        <div className="app__modal_wrapper3">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="app__modal_body">
            ...
            <div className='app__form_field_container'>
              <div className='w-full'>
                <div className='app__label_standard'>Item Holder (Optional):</div>
                {
                  editData?.hrm_user
                    ? <SearchUserInput
                        isMultiple={false}
                        selectedUsers={[editData?.hrm_user]}
