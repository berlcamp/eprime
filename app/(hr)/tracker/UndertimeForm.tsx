...
// Types
import type { Employee, UndertimePermitTypes, namesType } from '@/types'
...
const UndertimeForm = ({ hideModal }: ModalProps) => {
  ...
  const { register, formState: { errors }, reset, handleSubmit } = useForm<UndertimePermitTypes>({
    mode: 'onSubmit'
  })
  ...
  const handleCreate = async (formdata: UndertimePermitTypes) => {
    ...
    try {
      ...
      if (error) {
        void logError('Create Undertime Permit', 'hrm_request_trackers', JSON.stringify(newData), error.message)
      ...
      if (error2) {
        void logError('Create Undertime Permit Tracker Flow', 'hrm_tracker_flow', JSON.stringify({ tracker_id: data[0].id, user_id: currentUser.id, status: 'For Verification' }), error2.message)
        setToast('error', 'Saving failed, please reload the page and try again.')
          ...
          <div className='w-full px-4'>
            <div className='app__form_field_container'>
              <div className='w-full'>
                <div className='app__label_standard'>Time to leave on office</div>
                <div>
                  <input
