...
const UndertimeForm = ({ hideModal }: ModalProps) => {
  ...
  const handleCreate = async (formdata: UndertimePermitTypes) => {
    ...
    try {
      ...
      if (error2) {
        void logError('Create Undertime Permit Tracker Flow', 'hrm_tracker_flow', JSON.stringify({ tracker_id: data[0].id, user_id: currentUser.id, status: 'Request Created' }), error2.message)
        setToast('error', 'Saving failed, please reload the page and try again.')
