...
// Types
import type { Employee, ServiceRecordPrintRequestTypes, namesType } from '@/types'
...
const ServiceRecordPrintRequestForm = ({ hideModal }: ModalProps) => {
  ...
  const { register, formState: { errors }, reset, handleSubmit } = useForm<ServiceRecordPrintRequestTypes>({
    mode: 'onSubmit'
  })

  const onSubmit = async (formdata: ServiceRecordPrintRequestTypes) => {
    if (!user) {
  ...
  const handleCreate = async (formdata: ServiceRecordPrintRequestTypes) => {
    if (!user) return
    ...
    try {
      ...
      if (error) {
        void logError('Create Service Record Print Request', 'hrm_request_trackers', JSON.stringify(newData), error.message)
      ...
      if (error2) {
        void logError('Create Service Record Print Request Tracker Flow', 'hrm_tracker_flow', JSON.stringify({ tracker_id: data[0].id, user_id: currentUser.id, status: 'For Verification' }), error2.message)
        setToast('error', 'Saving failed, please reload the page and try again.')
  ...
  const handleNotifyReceiver = async (trackerId: string, receiverId: string, refCode: string) => {
    ...
    try {
      ...
      const { error: error3 } = await supabase
        ...
        .insert({
          message: `New Service Record Print Request #${refCode} has been forwarded to you for recommendation/approval.`,
          url: `/tracker/${refCode}`,
          type: 'Forwarded',
          user_id: receiverId,
            ...
            <div className='app__form_field_container'>
              <div className='w-full'>
                ...
                <div>
                  ...
                  {errors.purpose && <div className='app__error_message'>Purpose of Travel is required</div>}
                </div>
              </div>
            </div>
          </div>
...
export default ServiceRecordPrintRequestForm
