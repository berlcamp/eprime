...
const TravelForm = ({ hideModal }: ModalProps) => {
  ...
  const handleCreate = async (formdata: TravelTypes) => {
    ...
    try {
      ...
      if (error2) {
        void logError('Create Travel Authority Request Tracker Flow', 'hrm_tracker_flow', JSON.stringify({ tracker_id: data[0].id, user_id: currentUser.id, status: 'For Verification' }), error2.message)
        setToast('error', 'Saving failed, please reload the page and try again.')
