...
const TravelForm = ({ hideModal }: ModalProps) => {
  ...
  const handleCreate = async (formdata: LeaveTypes) => {
    ...
    try {
      ...
      if (error) {
        void logError('Create Travel Request', 'hrm_request_trackers', JSON.stringify(newData), error.message)
      ...
      if (error2) {
        void logError('Create Travel Request Tracker Flow', 'hrm_tracker_flow', JSON.stringify({ tracker_id: data[0].id, user_id: currentUser.id, status: 'Request Created' }), error2.message)
        setToast('error', 'Saving failed, please reload the page and try again.')
  ...
  const handleNotifyReceiver = async (trackerId: string, receiverId: string, refCode: string) => {
    ...
    try {
      ...
      const { error: error3 } = await supabase
        ...
        .insert({
          message: `New Travel Request #${refCode} has been forwarded to you for recommendation/approval.`,
          url: `/tracker/${refCode}`,
          type: 'Forwarded',
          user_id: receiverId,
                ...
                <div>
                  <select
                    ...
                    className='app__select_standard'>
                      ...
                      {
                        leaveTypes.map((item, index) => <option key={index} value={item}>{item}</option>)
                      }
                  </select>
