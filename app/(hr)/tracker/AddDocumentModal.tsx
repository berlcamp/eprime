...
export default function AddDocumentModal ({ hideModal }: ModalProps) {
  ...
  const handleCreate = async (formdata: DocumentTypes) => {
    ...
    try {
      ...
      if (error) {
        void logError('Create Request', 'hrm_request_trackers', JSON.stringify(newData), error.message)
      ...
      if (error2) {
        void logError('Create Request Tracker Flow', 'hrm_tracker_flow', JSON.stringify({ tracker_id: data[0].id, user_id: user.id, status: 'Request Created' }), error2.message)
        setToast('error', 'Saving failed, please reload the page and try again.')
      ...
      const updatedData = { id: data[0].id, creator: user, created_at: data[0].created_at, document_tracker_stickies: [], ...newData }
                ...
                <div className='w-full px-4'>
                  <div className='grid grid-cols-1 gap-4 mb-4'>
                    <div className='w-full'>
                      <div className='text-gray-600 font-medium text-sm mb-1 dark:text-gray-300'>Request Type<span className='italic text-xs text-gray-500'>(Required)</span>:</div>
                      <div>
                        <select
                        ...
                        {errors.type && <div className='mt-1 text-xs text-red-600 font-bold'>Request Type is required</div>}
                      </div>
                    </div>
                  </div>
                  {
                    type === 'Leave' &&
                        <div className='grid grid-cols-1 gap-4 mb-4'>
                          <div className='w-full'>
                            <div className='text-gray-600 font-medium text-sm mb-1 dark:text-gray-300'>From:</div>
                            <div>
                              <input
                                {...register('leave_from')}
                                type="date"
