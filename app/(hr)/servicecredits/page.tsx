...
const Page: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const result = await fetchServiceCredits(
        { filterKeyword },
        filterUrl,
        perPageCount,
        0
        ...
        <div>
          ...
          {!filterUrl && (
            <>
              ...
              <div className="app__warning_text">
                <span className="app__warning_title">Note:</span> Service Credit
                with employee/s cannot be deleted.
              </div>
          ...
          <div>
            <table className="app__table">
              ...
              <tbody>
                {!isDataEmpty &&
                  list.map((item: ServiceCreditTypes) => (
                    <tr key={uuid()} className="app__tr">
                      ...
                      <td className="hidden md:table-cell app__td">
                        ...
                        />
                        {item.status !== 'Expired' &&
                          countPending(item.hrm_service_credit_users ?? null) >
                            0 && (
                            <div className="text-red-500 font-semibold text-[10px]">
                              For Approval(
                              {countPending(
                                item.hrm_service_credit_users ?? null
                              )}
                              )
                            </div>
                          )}
