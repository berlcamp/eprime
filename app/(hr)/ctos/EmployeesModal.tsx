...
const EmployeesModal = ({ hideModal, ctoData }: ModalProps) => {
  ...
  const handleAddEmployee = async () => {
    ...
    const newData = { cto_id: ctoData.id, hrm_user_id: selectedItems[0].id, reference_code: refCode, is_approved: false }
  ...
  const handleApproveConfirmed = async () => {
    ...
    try {
      ...
      // update/add user cto balance
      const { data: ctoBalance, count, error3 } = await supabase
        .from('hrm_cto_user_balance')
        .select('*', { count: 'exact' })
      ...
      if (count > 0) {
        const currBalance = Number(ctoBalance[0].balance) + Number(ctoData.coc)
        await supabase
          .from('hrm_cto_user_balance')
          .update({ balance: currBalance })
          .eq('hrm_user_id', selectedRow?.hrm_user_id)
      } else {
        await supabase
          .from('hrm_cto_user_balance')
          .insert({ balance: ctoData.coc, hrm_user_id: selectedRow?.hrm_user_id })
      }
    ...
    <div className="app__modal_wrapper">
      <div className="app__modal_wrapper2_large">
        <div className="app__modal_wrapper3">
          ...
          {
            getStatus(ctoData.expiration) !== 'Expired' &&
              <div className="app__modal_header">
          ...
          <div className="app__modal_body">
            {/* Main Content */}
            <div className='pb-5'>
              <div className='app__warning_text2'><span className='app__warning_title'>Note:</span> Approved CTOs can no longer be deleted.</div>
