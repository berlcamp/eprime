...
import { fetchPlantillas } from '@/utils/fetchApi'
...
// Types
import type { PlantillaTypes } from '@/types'
...
const Page: React.FC = () => {
  ...
  const [list, setList] = useState<PlantillaTypes[]>([])
  const [filterPosition, setFilterPosition] = useState<string>('')
  const [filterSchool, setFilterSchool] = useState<string>('')
  const [filterUser, setFilterUser] = useState<string>('')

  const [perPageCount, setPerPageCount] = useState<number>(10)
  const [editData, setEditData] = useState<PlantillaTypes | null>(null)
  ...
  const fetchData = async () => {
    ...
    try {
      const result = await fetchPlantillas({ filterSchool, filterPosition, filterUser }, perPageCount, 0)
  ...
  const handleShowMore = async () => {
    ...
    try {
      const result = await fetchPlantillas({ filterSchool, filterPosition, filterUser }, perPageCount, list.length)
  ...
  const handleEdit = (item: PlantillaTypes) => {
    setShowAddModal(true)
    setEditData(item)
  }
    ...
    <div className="app__main">
      <div>
          <div className='app__title'>
            <Title title='Plantillas'/>
            <CustomButton
              containerStyles='app__btn_green'
              title='Create New Plantilla'
              btnType='button'
          ...
          <div>
            <table className="app__table">
              ...
              <tbody>
                {
                  !isDataEmpty && list.map((item: PlantillaTypes) => (
                    <tr
                      key={uuid()}
                      className="app__tr">
                      <td
                      ...
                      <th
                        ...
                        <div>
                          <div className="md:hidden app__td_mobile">
                            ...
                            <div><span className='app_td_mobile_label'>Date of Birth:</span> <span>{item.birthday && format(new Date(item.birthday), 'MMMM dd, yyyy')}</span></div>
                          </div>
                        </div>
                        {/* End - Mobile View */}
                      </th>
                      ...
                      <td
                        className="hidden md:table-cell app__td">
                          {item.birthday && format(new Date(item.birthday), 'MMMM dd, yyyy')}
                      </td>
                    </tr>
                  ))
                }
