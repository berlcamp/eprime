...
function SinglePrintNosa({
  ...
  const computed = React.useMemo(() => {
    ...
    const userName = [
      ...
      [selectedItem.hrm_user?.firstname, selectedItem.hrm_user?.middlename]
        .filter(Boolean)
        .join(" "),
    ]
      .filter(Boolean)
  ...
  return (
    <div
      className="print-nosa-container w-full max-w-full bg-white py-2 px-[10mm] min-h-0"
...
export const BulkPrintNosa = React.forwardRef<
  HTMLDivElement | null,
  BulkPrintNosaProps
>((props, ref) => {
  const { items, signatories } = props;

  return (
    <div className="invisible print:visible bulk-print-nosa" ref={ref}>
      <style>{`
          @media print {
            @page {
              size: 8.5in 13in;
      ...
      {items.map((item, i) => (
        ...
        >
          <SinglePrintNosa selectedItem={item} signatories={signatories} />
        </div>
      ))}
    </div>
  );
...
BulkPrintNosa.displayName = "BulkPrintNosa";
