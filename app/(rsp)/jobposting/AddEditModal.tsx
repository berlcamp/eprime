...
import { ConfirmModal, CustomButton, SearchUserInput, UserBlock } from '@/components'
...
const AddEditModal = ({ hideModal, editData }: ModalProps) => {
  ...
  const [user, setUser] = useState<namesType | null>(editData ? editData.hrm_user : null)
  ...
  const { register, formState: { errors }, reset, handleSubmit } = useForm<ItemTypes>({
    mode: 'onSubmit'
  })
  ...
  const handleCreate = async (formdata: ItemTypes) => {
    const newData = {
      ...
      implementing_unit_id: formdata.implementing_unit_id !== '' ? formdata.implementing_unit_id : null,
      ...
      date_of_original_appointment: new Date(formdata.date_of_original_appointment),
    ...
    try {
      ...
      if (error) {
        void logError('Create New Plantilla', 'hrm_items', JSON.stringify(newData), error.message)
      ...
      const hrmImplementingUnit = schools.find(p => p.id.toString() === formdata.implementing_unit_id)
      const hrmSchool = schools.find(p => p.id.toString() === formdata.school_id)
      const updatedData = { ...newData, implementing_unit: hrmImplementingUnit, hrm_school: hrmSchool, hrm_position: hrmPosition, hrm_user: user ?? null, id: data[0].id }
  ...
  const handleUpdate = async (formdata: ItemTypes) => {
    ...
    try {
      ...
      if (error) {
        void logError('Update Plantilla', 'hrm_items', JSON.stringify(newData), error.message)
        setToast('error', 'Saving failed, please reload the page and try again.')
        throw new Error(error.message)
      }
      ...
      const hrmImplementingUnit = schools.find(p => p.id.toString() === formdata.implementing_unit_id?.toString())
      const hrmSchool = schools.find(p => p.id.toString() === formdata.school_id?.toString())
      const items = [...globallist]
      const updatedData = { ...newData, implementing_uni: hrmImplementingUnit, hrm_school: hrmSchool, hrm_position: hrmPosition, hrm_user: user ?? null, id: editData.id }
  ...
  const handleConfirmedRemove = async () => {
    ...
    try {
      ...
      if (error) {
        void logError('Remove employee to Plantilla', 'hrm_items', '', error.message)
        setToast('error', 'Saving failed, please reload the page and try again.')
        throw new Error(error.message)
      }
              ...
              <div className="flex space-x-2 items-center justify-between border-b p-4 bg-orange-50">
                <div className='w-full'>
                  <div className="space-y-1">
                    <div className='font-medium text-sm text-gray-700'>Remove the current employee from this plantilla with the following reason:</div>
                    <div className="flex w-full space-x-2">
                      <textarea
                        value={reason}
                        onChange={e => setReason(e.target.value)}
                        className='text-sm py-1 px-2 text-gray-600 border border-gray-300 rounded-sm outline-none resize-none w-1/2' placeholder='Reason for removal'/>
                    </div>
                    <CustomButton
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="app__modal_body">
            <div className='flex flex-col lg:flex-row w-full items-start justify-between text-xs dark:text-gray-400'>
              ...
              <div className='w-full px-4'>
                    ...
                    <div className='app__label_standard'>Item Holder:</div>
                    {
                      editData?.hrm_user
                        ...
                        : <>
                            ...
                            <div className='text-gray-600 italic mt-1'>(Leaving blank means this item is Vacant)</div>
                          </>
                    }
                  </div>
                </div>
                <div className='app__form_field_container'>
                  <div className='w-full'>
                    <div className='app__label_standard'>Implementing Unit (IUs)</div>
                    <div>
                      <select
                        {...register('implementing_unit_id')}
                        value={selectedImplementingUnit}
                        onChange={e => setSelectedImplementingUnit(e.target.value)}
                        className='app__input_standard'>
                          <option value=''>Division</option>
                          {
                            schools.map((school, index) => <option key={index} value={school.id}>{school.name}</option>)
                          }
                      </select>
                    </div>
                  </div>
              ...
              <div className='w-full px-4'>
                {
                  !user
                    ? <div className="flex items-center">
                        ...
                        <div className="mx-4 my-4 text-gray-500 text-sm">No item holder selected</div>
                        <div className="flex-grow bg-gray-300 h-px"></div>
                      </div>
                    : <>
                        <div className="flex items-center">
                          <div className="flex-grow bg-gray-300 h-px"></div>
                          <div className="mx-4 my-4 text-gray-500 text-sm">Employee Details</div>
                          <div className="flex-grow bg-gray-300 h-px"></div>
                        </div>
                        <div className='app__form_field_container'>
                          <div className='w-full'>
                        ...
                        <div className='app__form_field_container'>
                          <div className='w-full'>
                            <div className='app__label_standard'>Civil Service Eligibility</div>
                            <div>
                              <input
                        ...
                        <div className='app__form_field_container'>
                          <div className='w-full'>
                            <div className='app__label_standard'>Date of Original Appointment</div>
                            <div>
                              <input
                        ...
                        <div className='app__form_field_container'>
                          <div className='w-full'>
                            <div className='app__label_standard'>Authorized Annual Salary</div>
                            <div>
                              <input
                        ...
                        <div className='app__form_field_container'>
                          <div className='w-full'>
                            <div className='app__label_standard'>Actual Annual Salary</div>
                            <div>
                              <input
