...
import { AddOpcrfModal } from './AddOpcrfModal'
...
const Page: React.FC = () => {
  ...
  const [modalIpcrfOpen, setModalIpcrfOpen] = useState(false)
  const [modalOpcrfOpen, setModalOpcrfOpen] = useState(false)
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const { data, count, error } = await supabase
        ...
        .select('*, positions:pms_ipcrf_positions(*,position:position_id(*))', {
          count: 'exact'
        })
      ...
      <div className="app__main">
        <div>
          <div className="app__title">
            <Title title="IPCRF/OPCRF Templates" />
            <Button onClick={() => setModalOpcrfOpen(true)} className="ml-auto">
              Create OPCRF Template
            </Button>
            <Button onClick={() => setModalIpcrfOpen(true)} className="ml-auto">
              Create IPCRF Template
            </Button>
          </div>
          ...
          <AddIpcrfModal
            isOpen={modalIpcrfOpen}
            onClose={() => setModalIpcrfOpen(false)}
          />
          <AddOpcrfModal
            isOpen={modalOpcrfOpen}
            onClose={() => setModalOpcrfOpen(false)}
          />
        </div>
      </div>
    </>
