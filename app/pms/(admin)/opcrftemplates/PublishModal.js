...
export default function PublishModal ({ selectedId, hideModal, handleUpdateList }) {
  ...
  const handlePublish = async () => {
    ...
    const totalWeight = data.objectives.reduce((partialSum, item) => partialSum + Number(item.weight), 0)

    if (totalWeight !== 100) {
      setErrorMessage('Total objectives weight must be equal to 100')
    } else {
      ...
      if (error) {
        ...
        // update list of parent component
        handleUpdateList({ is_published: 'true', id: selectedId })

        // success message
        setToast('success', 'Successfully published')
                ...
                <div className='w-full'>
                  {
                    errorMessage
                      ? (
                          <div className='text-red-500 font-medium text-sm mb-1 dark:text-gray-300'>Error: {errorMessage} </div>
                        )
                      : (
                          <div className='text-gray-600 font-medium text-sm mb-1 dark:text-gray-300'>Once you publish this template, you can no longer edit its details. Please confirm this. </div>
                        )
                  }
                </div>
              </div>
