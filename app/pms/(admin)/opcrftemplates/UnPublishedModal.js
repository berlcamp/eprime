...
export default function UnPublishedModal ({ selectedId, hideModal, handleUpdateList }) {
  ...
  const handleUnpublished = async () => {
    const { count } = await supabase
      .from('opcrfs')
      .select('*', { count: 'exact' })
      .eq('opcrf_template_id', selectedId)

    if (count > 0) {
      setErrorMessage('This template is already been used on employees OPCRFs and can no longer be unpublished. You can Archive this instead.')
    } else {
      ...
      if (error) {
        ...
        handleUpdateList({ is_published: '', id: selectedId })
