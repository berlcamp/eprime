...
export default function PublishModal({ selectedItem, hideModal }: ModalProps) {
  ...
  const handlePublish = async () => {
    const totalWeight = selectedItem.objectives.reduce(
      (partialSum, item) => partialSum + Number(item.weight),
      0
    )
              ...
              <div className="w-full">
                {errorMessage ? (
                  ...
                  <div className="text-gray-600 font-medium text-sm mb-1 dark:text-gray-300">
                    Once you publish this template, you can no longer edit its
                    details. Please confirm this.{' '}
                  </div>
                )}
              </div>
            </div>
