...
export default function Trainings ({ userId }: { userId: string }) {
  ...
  const onSubmit = async (formdata: WorkExperienceTypes) => {
    ...
    if (error) {
      void logError('Update L and D Trainings PDS', 'hrm_pds', JSON.stringify(newData), error.message)
          ...
          <div className="w-full">
            <div className='w-full px-4'>
              <div className="flex items-center">
                ...
                <div className="mx-4 my-4 text-gray-500 text-sm">Learning and Development (L&D) Interventions/Training Programs Attended</div>
                <div className="flex-grow bg-gray-300 h-px"></div>
              </div>
              <div className='w-full'>
                {
                  trainingsArray.length > 0 &&
                    <table className='app__table mb-4'>
                      <thead className='app__thead'>
                        <tr>
                          <th className='app__th'>Title of L&D Intervention/Training Programs</th>
                    ...
                    <form onSubmit={handleSubmit(onSubmitRow)} className="text-xs">
                      {
                        !showAddRow
                          ? <CustomButton
                              ...
                              title='Add Training Attended'
                              btnType='button'
                              handleClick={() => setShowAddRow(true)}
                              />
                          : <div className='w-2/3 space-y-4'>
                              <div className='mb-2 w-full'>
                                <div className='app__label_standard'>Title of L&D Intervention/Training Programs <span className='text-gray-500 text-[11px]'>(Write in full)</span>:</div>
                                <input
                                  {...register('title', { required: true })}
                                  type="text"
                                  className='app__input_standard'/>
                                {errors.title && <div className='app__error_message'>Title of L&D Intervention/Training Programs is required</div>}
                              </div>
                              <div className='mb-2 w-full'>
                              ...
                              <div className='mb-2 w-full'>
                                <div className='app__label_standard'>Type of LD (Managerial/Supervisory/Technical/etc):</div>
                                <input
                                  {...register('type', { required: true })}
                                  type="text"
                                  className='app__input_standard'/>
                                {errors.type && <div className='app__error_message'>Type of LD is required</div>}
                              </div>
                              <div className='mb-2 w-full'>
                                <div className='app__label_standard'>Conducted/ Sponsored By <span className='text-gray-500 text-[11px]'>(Write in full)</span>:</div>
                                <input
                                  {...register('sponsor', { required: true })}
                                  type="text"
                                  className='app__input_standard'/>
                                {errors.sponsor && <div className='app__error_message'>Conducted/ Sponsored By is required</div>}
                              </div>
