  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data } = await supabase
        ...
        .select('*, hrm_user:user_id(firstname,middlename,lastname),receiver:receiver_id(firstname,middlename,lastname), hrm_tracker_logs(*, hrm_user:user_id(firstname,middlename,lastname))', { count: 'exact' })
              ...
              <div className={`${flowList.length > 1 && (index + 1) < flowList.length ? 'text-gray-500' : 'text-gray-700 text-sm'} flex-1 ml-8 pb-4`}>
                ...
                }
                {
                  item.status === 'Forwarded' &&
                    <>
                    <div className='text-xs'>to {item.receiver.firstname} {item.receiver.middlename} {item.receiver.lastname}</div>
                    <div className='text-xs'>from {item.hrm_user.firstname} {item.hrm_user.middlename} {item.hrm_user.lastname}</div>
                    </>
                }
                <div className='ml-12'>
                  {
                    item.hrm_tracker_logs.length > 0 && item.hrm_tracker_logs.map((log, index) => (
                      <div key={index} className='text-[11px]'>
                        <span className='font-semibold'>{log.hrm_user.firstname} {log.hrm_user.middlename} {log.hrm_user.lastname} </span>
