    ...
    <div className={`w-full flex-col space-y-1 px-4 ${reply.reply_type !== 'system' ? 'py-4 border-t' : 'py-px'} text-xs text-gray-600 dark:bg-gray-800 dark:text-gray-400`}>
      <div className='w-full group'>
        <div className='flex items-center space-x-2'>
          <div className='flex flex-1 items-center space-x-2'>
            {
              reply.hrm_users.avatar_url !== null
                ? <div className='relative flex items-center justify-center bg-black overflow-hidden'>
                    <Image src={reply.hrm_users?.avatar_url} width={30} height={30} alt='user'/>
                  </div>
                : <Avatar round={false} size="30" name={reply.hrm_users.firstname}/>
            }
            <div>
              {
                reply.reply_type !== 'system' &&
                  <>
                    <div className='font-bold'>
                      <span>{reply.hrm_users.firstname} {reply.hrm_users.middlename} {reply.hrm_users.lastname}: </span>
                    </div>
                    <div
          ...
          {
            ((document.current_status === 'For Verification' && document.created_by === user.id) || (document.current_status === 'Forwarded' && document.receiver_id === user.id)) &&
