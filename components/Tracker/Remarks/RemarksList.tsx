...
import type { DocumentTypes, Employee, RemarksTypes } from '@/types'
...
export default function RemarksList({ reply, document }: ModalProps) {
  ...
  return (
    <div
      className={`w-full flex-col space-y-1 px-4 ${
        reply.reply_type !== 'system' ? 'py-4 border-t' : 'py-px'
    ...
    >
        ...
        <div className="flex items-center space-x-2">
          <div className="flex flex-1 items-center space-x-2">
            ...
            <div>
              ...
              {reply.is_private && (
                <div className="mt-1">
                  <span className="bg-orange-300 text-orange-900 px-1 py-px rounded-sm">
                    Private Note (Only you can view this)
                  </span>
                </div>
              )}
            </div>
          ...
          {((document.current_status === 'For Verification' &&
            document.created_by === user.id) ||
            (document.current_status === 'Forwarded' &&
        ...
        <div className="ml-12">
          ...
          <div className="mt-1">
            <div
              className={`mt-2 ${
                reply.is_private
                  ? 'bg-orange-100 p-2 border border-orange-200 rounded-sm'
                  : ''
              }`}
            >
      ...
      <div className="border-l ml-20">
        ...
        {!reply.is_private && <CommentBox document={document} reply={reply} />}
        {/* Comments */}
        {!reply.is_private &&
          reply.hrm_remarks_comments?.map((comment) => (
            <CommentsList comment={comment} reply={reply} key={uuid()} />
          ))}
        {showConfirmation && (
          <ConfirmModal
            btnText="Yes"
