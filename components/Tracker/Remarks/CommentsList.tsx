...
import type { CommentsTypes, DocumentTypes, RemarksTypes } from '@/types'
...
export default function CommentsList ({ document, comment, reply }: ModalProps) {
  ...
  const handleDeleteReply = async () => {
    try {
      ...
      const updatedCommentData = reply.hrm_remarks_comments.filter(item => item.id !== selectedId)
      const updatedData = { hrm_remarks_comments: [...updatedCommentData], id: reply.id }
      ...
      <div className='w-full group flex-col space-y-2 px-4 pt-2 text-xs text-gray-600 dark:bg-gray-800 dark:text-gray-400'>
        <div className='flex items-start space-x-2'>
          <div className='flex flex-1 items-start space-x-2'>
            {
              comment.hrm_users.avatar_url !== null
                ? <div className='relative flex items-center justify-center bg-black overflow-hidden'>
                    <Image src={comment.hrm_users?.avatar_url} width={30} height={30} alt='user'/>
                  </div>
                : <Avatar round={false} size="30" name={`${comment.hrm_users.firstname} ${comment.hrm_users.middlename} ${comment.hrm_users.lastname}`}/>
            }
            <div>
            <div className='font-bold'>{comment.hrm_users.firstname} {comment.hrm_users.middlename} {comment.hrm_users.lastname}:</div>
              <div
